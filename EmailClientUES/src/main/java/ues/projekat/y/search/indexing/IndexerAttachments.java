...
public class IndexerAttachments {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 2) {
			try{
				...
				//new File dataDir
				dataDirAttach = new File(rb.getString("dataDirAttach"));
			}catch(Exception e1){
				for(String arg :args)
			...
			indexDirAttach = new SimpleFSDirectory(new File(args[0]));
			dataDirAttach = new File(args[1]);
		}
		//pocetak indeksiranja
		long start = new Date().getTime();
		//broji koliko je fajlova indeksirao
		int numIndexed = index(indexDirAttach, dataDirAttach);
		//kraj indeksiranja
	...
	private static void indexFile(IndexWriter writer, File f)throws IOException {
		...
		try {
			...
			String sql = "select attachment_id, mime_type, name, path from attachments";
